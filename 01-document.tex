...
\section{Introduction}
    ...
    Departing from the continuous Boltzmann equation~\cite{2011-HarrisS-Dover}, the derivation of discrete velocity sets,  i.e.,
    the lattice vectors, was considered as a quadrature problem, in the senses that (i)~weights  $w_i$  for  lattice  velocities
    $\mathbf{\xi}_i$  would  be  found  so   that   moments   of   the   discrete   equilibrium   mass   distribution   function
    $f^{eq}(\mathbf{\xi}_i)$ would exactly match its continuous counterpart, and (ii)~to  warrant  even-ranked  velocity  tensor
    isotropy, which, in turn, translates into isotropic fluid transport properties.
