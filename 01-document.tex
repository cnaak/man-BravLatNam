...
\section{Introduction}
    ...
    considered as a quadrature problem, in the senses that (i)~weights $w_i$ for lattice velocities  $\mathbf{\xi}_i$  would  be
    found so that moments of the discrete equilibrium mass distribution function $f^{eq}(\mathbf{\xi}_i)$  would  exactly  match
    its continuous counterpart, and (ii)~to warrant even-ranked velocity  tensor  isotropy,  which,  in  turn,  translates  into
    isotropic fluid transport properties.
