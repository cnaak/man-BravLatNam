...
\section{Introduction}
    ...
    lattice \emph{velocity count}, as in D1Q3, D2Q9, and D3Q15, etc.~\cite{1992-QianYH+LallemandP-EurophysLett}---that seems  to
    be  the  most  prevalent  lattice  naming  system  to  date,   although   notable   exceptions   appear   long   after   the
    paper~\cite{1991-QianYH+LallemandP-AdvKinTheoContMech} came out in 1991.
    ...
    Karlin~\cite{2006-ChikatamarlaSS+KarlinIV-PhysRevLett} approached the problem of LB stencil  construction  from  a  discrete
    form  of  Boltzmann's  H-theorem---in  which  the  $-H$  quantity  is  a  sort  of  generalized  thermodynamic  entropy  for
    non-equilibrium states in the Boltzmann Gas Limit (BGL)~\cite{2011-HarrisS-Dover}---

    As the immediate examples of~\cite{2006-PhilippiPC+SurmasR-PhysRevE} evidence, as soon as one allows for  including  several
    energy levels (but not necessarily all, nor in monotonic order)  in  a  Bravais  lattice,  \emph{velocity  count  no  longer
    uniquely} identifies lattices, and thus \emph{velocity count-based lattice naming schemes are  bound  to  be  ambiguous  and
    require additional information as to uniquely identify the lattice.\/} Usually, the additional  information  is  laying  out
    \emph{all} velocity vectors, whether by  energy  level  listings,  a  quiver-like  lattice  picture,  or  tabulated  lattice
    velocities (and weights)---all of which seem, to varying degrees, excessively wordy and lengthy. Moreover, the very need for
    providing additional information as to uniquely identify an already named lattice seems to  defeat  the  purpose  of  naming
    them.

    %---------------------------------------------------------------------------------------------------------------------------
    \subsection{Higher-order lattice proliferation}

    The onset of sistematic techniques for LB stencils fabrication, allied to the expansion of LB  simulation  applications  and
    ...
    in velocity count-based naming schemes, as well as prompted the appearance of yet more lattice naming variety.

    The following `DdVb' lattices were derived by  Ortiz~\cite{2007-OrtizCEP-DrUFSC},  using  prescribed  abscissas  quadrature:
    second-order hexagonally regular (Bravais)  D2V7;  third-order  irregular,  i.e.,  not  space-filling,  D2V12;  fourth-order
    irregular D2V19, D2V20, D2V21, and regular D2V37; fifth-order irregular D2V28a,  D2V28b,  and  regular  D2V53a  and  D2V53b;
    sixth-order regular D2V81. In three-dimensional Euclidean space, the following: second-order  irregular  D3V13,  third-order
    irregular D3V27, and fourth-order irregular D3V52 and D3V53.
    ...
    Discussion on a plurality of lattices and lattice operations, based on

    yet  another  higher-order  LB  theory  that  allows
    higher-\emph{dimension} lattices to be obtained from lower-dimensional ones (from as low as 1-D lattice basis),

    takes  place
    in~\cite{2009-ChikatamarlaSS+KarlinIV-PhysRevE}; noteworthy ones are D3Q41, D3Q125, beyond several low-order ones in  three-
    and in two- Euclidean dimensional spaces. The reference also add the `-ZOT'  suffix---for  zero-one-three,  a  reference  to
    their `\{$0$, $\pm 1$, $\pm 3$\}' parent 1-D lattice.



    % As conciseness is preferred over long-windedness, and ambiguity is generally frowned upon, Therefore, proposing an
    % unambiguous, energy-level based, Bravais lattice naming scheme for  LB stencils is the goal of this work.
